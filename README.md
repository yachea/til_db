...
- FK 가 없는 경우에 데이터 오류가 발생하며 데이터 추가 안됨.
...
```sql
UPDATE 테이블명 SET 칼러명 = 값, 칼럼명 = 값 WHERE 조건;
UPDATE 테이블명 SET 칼러명 = 값, 칼럼명 = 값 WHERE 필드이름=값;

-- 사용자 id 1 의 email 을 변경하기
update authors set email = "hong@mgail.com" where id=1;
select * from authors;
```
...
- 개발자가 가장 많이 사용함.
- 특정한 데이터에서 원하는 결과를 도출 해 내는 작업이다.
- 알고리즘 시험에 늘 나옴.
- 모든 데이터 조회하기
...
- 날짜와 시간을 저장.
- 옵션을 주면 m 지정시 소수점 microsecondes 단위까지 저장.
- YYYY-MM-DD HH:MM:SS
- 가장많이 사용
- 주문시간, 글 작성 시간 등
